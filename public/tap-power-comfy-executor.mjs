...
const workflowFile = process.env.TAP_POWER_COMFY_WORKFLOW;
if (!raw) throw new Error("TAP_POWER_JOB was not supplied by the worker");
if (!workflowFile) throw new Error("Set TAP_POWER_COMFY_WORKFLOW to an exported ComfyUI API workflow JSON file");
const job = JSON.parse(raw);
const prompt = String(job.prompt || "").trim();
if (!prompt) throw new Error("The image job has no prompt");
const negative = process.env.TAP_POWER_COMFY_NEGATIVE_PROMPT || "blurry, low quality, watermark, text";
...
const base = (process.env.TAP_POWER_COMFY_URL || "http://127.0.0.1:8188").replace(/\/$/, "");
...
const until = Date.now() + Number(process.env.TAP_POWER_TIMEOUT_MS || 1800000);
