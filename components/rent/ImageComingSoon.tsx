...
export function ImageComingSoon() {
  ...
  const submit = async () => { if (!prompt.trim()) return; setBusy(true); setMessage("Fund 0.01 SOL in escrow to begin…"); setImage(""); try { const job = await submitJob({ prompt, modelSource: "managed:image", budget: "0.01" }); if (job.error || !job.job) throw new Error(job.error || "Escrow funding failed."); setMessage("Escrow funded. Processing image…"); const response = await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind: "image", prompt, jobId: job.job.id }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Image generation failed."); setImage(data.dataUrl); setMessage(data.cost != null ? `Image generated · ${Number(data.cost).toFixed(4)} USD provider cost` : "Image generated."); } catch (error) { setMessage(error instanceof Error ? error.message : "Image generation failed."); } finally { setBusy(false); } };
