...
export function AdminOverview() {
  ...
  return <section className="mt-4 space-y-4"><Panel><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow">Administrator · verified</p><h2 className="mt-3 font-display text-2xl text-ivory">Network operations</h2></div><StatusPill live>{overview.mongo ? "MongoDB live" : "Local store"}</StatusPill></div><div className="mt-7 grid gap-4 sm:grid-cols-4"><Metric label="Wallets" value={String(overview.wallets.length)} /><Metric label="Workers" value={String(overview.workers.length)} /><Metric label="Jobs" value={String(overview.jobs.length)} /><Metric label="Settlements" value={String(overview.ledger.length)} /></div></Panel>
